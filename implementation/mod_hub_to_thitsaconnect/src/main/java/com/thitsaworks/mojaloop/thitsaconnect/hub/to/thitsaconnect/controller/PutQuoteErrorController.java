...
@RestController
public class PutQuoteErrorController {

    private static final Logger LOG = LoggerFactory.getLogger(PutQuoteErrorController.class);

    @Autowired
    private RedisPayerService redisPayerService;

    @RequestMapping(value = "/quotes/{id}/error", method = RequestMethod.PUT)
...
//            @RequestHeader("FSPIOP-HTTP-METHOD") String fspiopHttpMethod,
            @PathVariable("id") String quoteId, @Valid @RequestBody ErrorInformationResponse request) {

        LOG.info("Put Quote error request info from Hub to Payer thitsaconnect quoteId:" + quoteId);
        LOG.info("Put Quote error request info from Hub to Payer thitsaconnect request:" + new Gson().toJson(request));

        HubToPayerOutput hubToPayerOutput = redisPayerService.getDataWithTransferId(quoteId);

        if (hubToPayerOutput == null) {

            LOG.error("Put Quote Error Redis Data Not Found with ID: " + quoteId);
